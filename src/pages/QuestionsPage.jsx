...
export default function QuestionsPage() {
    ...
    useEffect(() => {
        ...
        const unsubS = onSnapshot(qStats, (snap) => {
            // FIXED: Removed 'd' here. Only counting a, b, c.
    ...
    if (!question) return (
        <Layout>
            <div className="text-center text-slate-400 p-8 bg-slate-800 rounded-2xl border border-slate-700 shadow-xl">
                <h2 className="text-xl font-bold text-white mb-2">Heute keine Frage</h2>
        ...
        return (
            <Layout>
                ...
                <div className="space-y-4">
                    ...
                    {Object.entries(question.options).sort().map(([key, text]) => {
                        ...
                        let cardStyle = "bg-slate-800 border-slate-700 text-slate-400 opacity-80";
                        ...
                        return (
                            <div key={key} className={`relative p-4 rounded-xl border transition-all duration-300 ${cardStyle} ${ring}`}>
                                ...
                                <div className="h-2.5 w-full bg-slate-900/50 rounded-full overflow-hidden border border-white/5">
                                    <div className={`h-full transition-all duration-1000 ${barColor}`} style={{ width: `${percent}%` }}></div>
                                </div>
                                {isCorrectKey && (
